...
REM 32bit path    : https://www.python.org/ftp/python/3.14.0/python-3.14.0b2.exe
REM 64bit path    : https://www.python.org/ftp/python/3.14.0/python-3.14.0b2-amd64.exe
REM Date          : 20250529
REM -----------------------------------------------------------------------------------
set major=3
set minor=14
...
:start_this
    setlocal
    set _value="n"
    set /p _value=Are you sure installing Python-%major%.%minor%.%patch% Beta Candidate 2 x86-64 or x86 (y/[n]) ?
    if /i "%_value%" NEQ "y" goto end

:: This batch file will show details Windows 10, and install Python v3.12.0
:section_0
    setlocal
...
:section_4
    ...
    if /i "%processor_architecture%"=="arm64" (
        ...
        if exist %badmin% (
            ...
            echo. We dont know where bitsadmin is located.
            echo. line 181
            goto end
        )
    ) 
    ...
    if /i "%processor_architecture%"=="x86" (
        rem 
        if exist %badmin% (
            echo. Bitsadmin 32bit is isinstalled on your windows system.
            echo. Will download Python 3 Software.
            echo.
            goto section_5
        ) else (
            echo. We dont know where bitsadmin 32 bit is located.
            echo. line 207
            goto end
        )
    )
...
::could expand this are to include more of architecture types.
rem expand on the other types
    echo. 40%% Completed.
    echo.
...
:section_15
    ...
    echo. ============================
    echo set PATH=C:\Python%major%%minor%%patch%%release%;%PATH% > ..\runme%major%%minor%%patch%%release%.bat
    ...
    call ..\runme%major%%minor%%patch%%release%.bat
    echo.
