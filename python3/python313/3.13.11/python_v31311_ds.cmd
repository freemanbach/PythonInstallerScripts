...
REM x8632bit path    : https://www.python.org/ftp/python/3.13.11/python-3.13.11.exe
REM AMD64bit path    : https://www.python.org/ftp/python/3.13.11/python-3.13.11-amd64.exe
REM ARM64bit path    : https://www.python.org/ftp/python/3.13.11/python-3.13.11-arm64.exe
REM Date             : 20251222
REM -----------------------------------------------------------------------------------
set major=3
set minor=13
...
:section_3_1
    if exist "python-3.11.0-*.<" (
        echo. Found existing version of Python 3.11
        del python-3.11.*-*.exe
        del python-3.11.*.exe
        echo. Deleting existing version of Python
        echo.
    ) else (
        echo. No existing version of python 3.11
        echo.
    )
    echo. 10%% Completed.
...
:section_3_2
    if exist "python-3.10.*.<" (
        echo. Found existing version of Python 3.10
        del python-3.10.*-*.exe
        del python-3.10.*.exe
        echo. Deleting existing version of Python
        echo.
    ) else (
        echo. No existing version of python 3.10.
        echo.
    )
    echo. 15%% Completed.
...
:section_3_3
    if exist "python-3.9.*.<" (
        ...
        echo. No existing version of python 3.9
        echo.
    )
    echo. 20%% Completed.
...
:section_6
    ...
    if /i "%processor_architecture%"=="arm64" (
        if exist python-%major%.%minor%.%patch%-arm64.exe (
...
:section_13
    ...
    echo. Super Comprehensive packages for Data Science
    echo. ==============================================
    echo.
    echo. Press ENTER in this section if and when it seem to be pausing too long.
    echo.
    ...
    timeout /t 2 > nul
    C:\Python%major%%minor%%patch%\Scripts\pip.exe install --user pipx bs4 requests
    timeout /t 2 > nul
    C:\Python%major%%minor%%patch%\Scripts\pip.exe install --user numpy scipy
    timeout /t 2 > nul
    C:\Python%major%%minor%%patch%\Scripts\pip.exe install --user ta statsmodels
    timeout /t 2 > nul
    C:\Python%major%%minor%%patch%\Scripts\pip.exe install --user pandas polars matplotlib
    timeout /t 2 > nul
    C:\Python%major%%minor%%patch%\Scripts\pip.exe install --user seaborn plotly pandas_datareader
    timeout /t 2 > nul
    C:\Python%major%%minor%%patch%\Scripts\pip.exe install --user pymc3 pingouin
    timeout /t 2 > nul
    C:\Python%major%%minor%%patch%\Scripts\pip.exe install --user prophet yfinance
    timeout /t 2 > nul
    ...
    timeout /t 2 > nul
    C:\Python%major%%minor%%patch%\Scripts\pip.exe install --user gensim pycaret
