...
REM x8632bit path    : https://www.python.org/ftp/python/3.13.0/python-3.13.0rc2.exe
REM AMD64bit path    : https://www.python.org/ftp/python/3.13.0/python-3.13.0rc2-amd64.exe
REM ARM64bit path    : https://www.python.org/ftp/python/3.13.0/python-3.13.0rc2-arm64.exe
REM Date             : 2024.09.14
REM -----------------------------------------------------------------------------------
set major=3
set minor=13
...
:start_this
    ...
    set /p _value=Are you sure installing Python-3.13.0 Beta 4 release x86_64, x86 or ARM64 (y/[n]) ?
...
:check_arch
    ...
    if /i "%processor_architecture%"=="x86" (
            ...
            %badmin% /transfer PythonDownload /download /priority normal https://www.python.org/ftp/python/%major%.%minor%.%patch%/python-%major%.%minor%.%patch%%release%.exe C:\Users\%USERNAME%\Downloads\python-%major%.%minor%.%patch%%release%.exe
        ) 
    if /i "%processor_architecture%"=="amd64" (
            rem Run 64bit downloader
            %badmin% /transfer PythonDownload /download /priority normal https://www.python.org/ftp/python/%major%.%minor%.%patch%/python-%major%.%minor%.%patch%%release%-amd64.exe C:\Users\%USERNAME%\Downloads\python-%major%.%minor%.%patch%%release%-amd64.exe
        )
    if /i "%processor_architecture%"=="arm64" (
            rem Run 64bit downloader
            %badmin% /transfer PythonDownload /download /priority normal https://www.python.org/ftp/python/%major%.%minor%.%patch%/python-%major%.%minor%.%patch%%release%-arm64.exe C:\Users\%USERNAME%\Downloads\python-%major%.%minor%.%patch%%release%-arm64.exe
        )
    echo. 40%% Completed.
    echo.
...
:section_8
    ...
    if /i "%processor_architecture%"=="arm64" (
        C:\Users\%USERNAME%\Downloads\python-%major%.%minor%.%patch%%release%-arm64.exe /quiet /passive InstallAllUsers=0 TargetDir=C:\Python%major%%minor%%patch%%release% AssociateFiles=1 CompileAll=1 PrependPath=0 Shortcuts=0 Include_doc=1 Include_debug=0 Include_dev=1 Include_exe=1 Include_launcher=1 InstallLauncherAllUsers=1 Include_lib=1 Include_pip=1 Include_symbol=0 Include_tcltk=1 Include_test=1 Include_tools=1
        timeout /t 2 > nul
    )
    echo. 60%% Completed.
    echo.
...
:section_15
    ...
    REM Run this Auto generated --runme.bat-- inside a DOS prompt before using Python 3.13.X
    echo set PATH=C:\Python%major%%minor%%patch%%release%;%PATH% > C:\Users\%USERNAME%\runme.bat
    ...
    if exist C:\Users\%USERNAME%\Downloads\python-%major%.%minor%.%patch%%release%-arm64.exe (
        REM deleting 64 bit installer
        echo. deleting installation file
        del /s C:\Users\%USERNAME%\Downloads\python-%major%.%minor%.%patch%%release%-arm64.exe >nul 2>&1
    )
